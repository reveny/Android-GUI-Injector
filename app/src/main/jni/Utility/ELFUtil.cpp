...
uintptr_t ELFParser::GetSymbolOffset32(uintptr_t entryAddr, Elf32_Ehdr* entryElf, const char* symbolName) {
    ...
    Elf32_Shdr* sections = (Elf32_Shdr*)((uintptr_t)entryAddr + entryElf->e_shoff);
...
uintptr_t ELFParser::GetSymbolOffset(const char* elfPath, const char* symbolName) {
    ...
    entryRaw = mmap(NULL, static_cast<size_t>(elfStat.st_size), PROT_READ, MAP_SHARED, fd, 0);
    ...
    munmap(entryRaw, static_cast<size_t>(elfStat.st_size));
    close(fd);
